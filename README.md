...
RabbitMQ 계정과 vhost 권한은 컨테이너 시작 시 `.env`의
`RABBITMQ_DEFAULT_USER`, `RABBITMQ_DEFAULT_PASS`, `RABBITMQ_DEFAULT_VHOST`로
생성하거나 갱신한다. `rabbitmq/definitions.json`은 Exchange, Queue, Binding
등 topology만 관리한다.

RabbitMQ 계정 값을 변경한 뒤 컨테이너를 재생성하면 기존 볼륨을 유지한
상태에서도 새 비밀번호가 적용된다.
